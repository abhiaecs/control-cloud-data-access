...
ResultSet r2=st2.executeQuery("SELECT *FROM aareg where uname='"+username+"' ");
if(!r2.next()){


ResultSet r=st1.executeQuery("SELECT MAX(no) FROM aareg");
if(r.next()){
	i=r.getInt(1);
	if(i>=0){ i++;}}


int q= st.executeUpdate("insert into aareg values("+i+",'"+name+"','"+username+"','"+pasword+"','"+eid+"', '"+cno+"','None')");
...
int mkey=rn.nextInt(100000);
 ...
 st.executeUpdate("insert into register1 values("+i+",AES_ENCRYPT('"+name+"',"+mkey+"),AES_ENCRYPT('"+username+"',"+mkey+"),AES_ENCRYPT('"+pasword+"',"+mkey+"),AES_ENCRYPT('"+company+"',"+mkey+"),AES_ENCRYPT('"+email+"',"+mkey+"), AES_ENCRYPT('"+mobileno+"',"+mkey+"),AES_ENCRYPT('"+department+"',"+mkey+"),AES_ENCRYPT('"+salary+"',"+mkey+"),"+mkey+")");*/
	...
	response.sendRedirect("aaregister.jsp?unm=fail");
}
}
catch(Exception e)
{
